	...
	while (done) {
		...
		if (ev.type == ALLEGRO_EVENT_KEY_DOWN) {
			...
			case ALLEGRO_KEY_LEFT:
				keys[left] = true; // tuşa basılı tutma işlemi
				break;
			case ALLEGRO_KEY_RIGHT:
				keys[right] = true;  // tuşa basılı tutma işlemi
				break;
			}
			...
			case ALLEGRO_KEY_LEFT:
				keys[left] = false; // tuşu bırakma
				break;
			case ALLEGRO_KEY_RIGHT:
				keys[right] = false;  // tuşu bırakma
				break;
		...
		while (myball.x >= 1004) {  // Sağ tarafın sınırı için
			myball.x -= 10;
		}
		while (myball.x <= 20) {  // Sol tarafın sınırı için
			myball.x += 10;
		}
