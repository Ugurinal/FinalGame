...
int main(int argc, char **argv) {
	...
	while (done) {
		...
		myball.vely += 0.5;
		printf("%f %f\n", myball.y, myball.vely);
		if (myball.y + myball.radius > height && myball.vely > 0) { //Ball has hit the floor and is moving downward
		...
		if (ev.type == ALLEGRO_EVENT_KEY_DOWN) {
			...
			case ALLEGRO_KEY_LEFT:
				keys[left] = true; // tuþa basýlý tutma iþlemi
				break;
			case ALLEGRO_KEY_RIGHT:
				keys[right] = true;  // tuþa basýlý tutma iþlemi
				break;
			}
			...
			case ALLEGRO_KEY_LEFT:
				keys[left] = false; // tuþu býrakma
				break;
			case ALLEGRO_KEY_RIGHT:
				keys[right] = false;  // tuþu býrakma
				break;
		...
		while (myball.x >= 1004) {  // Sað tarafýn sýnýrý için
			myball.x -= 10;
		}
		while (myball.x <= 20) {  // Sol tarafýn sýnýrý için
			myball.x += 10;
		}
		...
		if (redraw && al_is_event_queue_empty(event_queue)) {
			...
			al_flip_display();
			al_clear_to_color(al_map_rgb(0, 0, 0)); // bufferý temizleyip arkadaki renkleri sýfýrlýyor önceki hareketi ekranda kalmýyor.
		}
